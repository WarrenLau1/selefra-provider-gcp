...
func (x *TableGcpComputeSslPoliciesGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			...
			it := c.GcpServices.ComputeSslPoliciesClient.List(ctx, req)
			for {
				resp, err := it.Next()
...
func (x *TableGcpComputeSslPoliciesGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		...
		table_schema_generator.NewColumnBuilder().ColumnName("custom_features").ColumnType(schema.ColumnTypeStringArray).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("profile").ColumnType(schema.ColumnTypeString).Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("warnings").ColumnType(schema.ColumnTypeJSON).Build(),
